...
public class SolutionCDIII {
    ...
    public boolean helper(int[] stones, int i, int step) {
        if (i == stones.length-1) return true;
        for (int j = i+1; j < stones.length; j++) {
            if (stones[j] <= stones[i] + step + 1 && stones[j] >= stones[i] + step -1) {
                if (helper(stones, j, stones[j]-stones[i])) return true; // actual step passed: stones[j]-stones[i]
            }
        }
        return false;
    }
